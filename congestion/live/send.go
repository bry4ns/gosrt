...
func NewSender(config SendConfig) congestion.Sender {
	...
	if config.MaxBW > 0 {
		s.maxBW = float64(config.MaxBW)
	} else {
		s.maxBW = 0 // Se estimara dinamicamente a 10x la tasa de entrada en ACK()
	}
	s.pktSndPeriod = (s.avgPayloadSize + 16) * 1_000_000 / 128 / 1024 / 1024 // 1 Gbit/s inicial por defecto
...
func (s *sender) ACK(sequenceNumber circular.Number) {
	...
	// Recalcular pktSndPeriod considerando OverheadBW
	targetBW := s.maxBW
	if targetBW <= 0 {
		// Modo automatico: estimar maxBW a 10x el bitrate de entrada (minimo 1 Mbps para evitar congelamiento)
		estimatedInput := s.rate.estimatedInputBW
		if estimatedInput < 125000 { // 1 Mbps en bytes/s
			estimatedInput = 125000
		}
		targetBW = estimatedInput * 10
	}

	effectiveBW := targetBW
	if s.overheadBW > 0 {
		effectiveBW = targetBW * (1.0 + s.overheadBW/100.0)
	}

	s.pktSndPeriod = (s.avgPayloadSize + 16) * 1000000 / effectiveBW
}
